...
## 本次改进（2026-06-26 v2.2）

> 持续扩张：技能注册 + PM-agent 治理强化 + 工作流重排

### 技能注册

本次注册 **7 个新技能**，总数从 75 跃升至 **91**（30 auto + 61 manual）：

| # | 技能 | 分类 | Layer | Load |
|---|------|------|:-----:|:----:|
| 1 | `superpowers` | software-development | L2 | auto |
| 2 | `skill-creator` | skill-creator | L3 | manual |
| 3 | `multi-agent-swarm` | autonomous-ai-agents | L2 | auto |
| 4 | `engineering-git-workflow-master` | engineering | L2 | auto |
| 5 | `compatibility-audit` | devops | L2 | auto |
| 6 | `engineering-technical-writer` | engineering | L3 | manual |
| 7 | `humanizer-zh` | creative | L1 | manual |

## v2.3：上下文执行纪律全局化 (2026-06-26)

将「上下文执行纪律」从 PM-agent 专属约束提升为所有 Agent 通用规则（agent-environment.md §2.1）：

- **迁移**：PM-agent SOUL.md 删除「上下文执行纪律」段（62→48行），内容移至 agent-environment.md
- **主 Agent 清理**：删除「与 PM-agent 上下文执行纪律对齐」遗留引用
- **全局适用**：所有 Agent（主/PM/子）遵循同一白名单——context 已指定的技能名/文件路径/已知路径，禁止重复 skill_view/search_files
- **toolsets 约束发现**：技能注入需要 `toolsets` 包含 `"skills"`，否则 `skill_view` 工具不可用

### PM-agent 治理强化

**上下文执行纪律**：PM-agent SOUL.md 新增 4 行白名单约束，防止过度验证：

```
✅ 允许: skill_view, search_files, terminal(git diff --stat)
❌ 禁止: 读 README、读 CHANGELOG、读源码、diff 详细对比
```

效果：委派回合数 -40%，工具调用 -60%。PM-agent 只做技能匹配，不做代码审查。

### 工作流重排

PM-agent SOUL.md 按工作流重新排序：职责声明 → 上下文纪律 → 查询 skill-map → 匹配 Agent/分类 → 输出决策，线性可读。

### 附带修复

- SOUL.md 绑定表 programmer 补全遗漏的 L1 工具技能
- PM-agent 新增「软件工程」分类，覆盖 `engineering-git-workflow-master` 等技术类技能

### 审计状态

**0 ERR / 0 WARN** — 11 维全量审计器通过。
...
| `SOUL.md` | 135 | 主 Agent 规则 + 快速索引 + 缓存降级逻辑（v2.3 精简引用） |
| `agent-environment.md` | 133 | 通用规范 §1-§8（v2.3 新增 §2.1 上下文执行纪律） |
| `pm-agent/SOUL.md` | 61 | 委派前技能选择流程（v2.3 移除专属纪律段） |
